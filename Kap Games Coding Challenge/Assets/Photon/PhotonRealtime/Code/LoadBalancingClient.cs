...
namespace Photon.Realtime {
    ...
    public class LoadBalancingClient : IPhotonPeerListener {
        ...
        private static readonly Dictionary<ConnectionProtocol, int> ProtocolToNameServerPort = new() { { ConnectionProtocol.Udp, 5058 }, { ConnectionProtocol.Tcp, 4533 }, { ConnectionProtocol.WebSocket, 9093 }, { ConnectionProtocol.WebSocketSecure, 19093 } }; //, { ConnectionProtocol.RHttp, 6063 } };
        ...
        private string GetNameServerAddress() {
            ...
            if (NameServerPortInAppSettings != 0) {
                DebugReturn(DebugLevel.INFO, string.Format("Using NameServerPortInAppSettings: {0}", NameServerPortInAppSettings));
        ...
        public virtual bool ConnectUsingSettings(AppSettings appSettings) {
            ...
            if (IsUsingNameServer) {
                ...
                if (!LoadBalancingPeer.Connect(NameServerAddress, ProxyServerAddress, AppId, TokenForInit)) return false;
                ...
                if (!LoadBalancingPeer.Connect(MasterServerAddress, ProxyServerAddress, AppId, TokenForInit)) return false;
        ...
        public bool ConnectToRegionMaster(string region) {
            ...
            if (State == ClientState.Authenticating) {
                if (LoadBalancingPeer.DebugOut >= DebugLevel.INFO) DebugReturn(DebugLevel.INFO, "ConnectToRegionMaster() will skip calling authenticate, as the current state is 'Authenticating'. Just wait for the result.");
                return true;
            }
        ...
        private bool Connect(string serverAddress, string proxyServerAddress, ServerConnection serverType) {
            ...
            if (AuthMode != AuthModeOption.Auth && serverType != ServerConnection.NameServer && TokenForInit == null) {
                DebugReturn(DebugLevel.ERROR, "Connect() failed. Can't connect to " + serverType + " with Token == null in AuthMode: " + AuthMode);
                return false;
            }

            // connect might fail, if the DNS name can't be resolved or if no network connection is available, etc.
            bool connecting = LoadBalancingPeer.Connect(serverAddress, proxyServerAddress, AppId, TokenForInit);
        ...
        public void Disconnect(DisconnectCause cause = DisconnectCause.DisconnectByClientLogic) {
            if (State == ClientState.Disconnecting || State == ClientState.PeerCreated) {
                DebugReturn(DebugLevel.INFO, "Disconnect() call gets skipped due to State " + State + ". DisconnectedCause: " + DisconnectedCause + " Parameter cause: " + cause);
                return;
            }
        ...
        private bool CallAuthenticate() {
            if (IsUsingNameServer && Server != ServerConnection.NameServer && (AuthValues == null || AuthValues.Token == null)) {
                DebugReturn(DebugLevel.ERROR, "Authenticate without Token is only allowed on Name Server. Connecting to: " + Server + " on: " + CurrentServerAddress + ". State: " + State);
                return false;
            }
            ...
            if (AuthMode == AuthModeOption.Auth) {
                ...
                ConnectionProtocol targetProtocolPastNameServer = ExpectedProtocol != null ? (ConnectionProtocol)ExpectedProtocol : LoadBalancingPeer.TransportProtocol;
                return LoadBalancingPeer.OpAuthenticateOnce(AppId, AppVersion, AuthValues, CloudRegion, EncryptionMode, targetProtocolPastNameServer);
            }
        }
        ...
        public bool OpFindFriends(string[] friendsToFind, FindFriendsOptions options = null) {
            ...
            List<string> friendsList = new(friendsToFind.Length);
        ...
        public bool OpJoinRandomOrCreateRoom(OpJoinRandomRoomParams opJoinRandomRoomParams, EnterRoomParams createRoomParams) {
            ...
            bool sending = LoadBalancingPeer.OpJoinRandomOrCreateRoom(opJoinRandomRoomParams, createRoomParams);
            if (sending) {
        ...
        public bool OpJoinRoom(EnterRoomParams enterRoomParams) {
            ...
            bool sending = LoadBalancingPeer.OpJoinRoom(enterRoomParams);
            if (sending) {
                lastJoinType = enterRoomParams.JoinMode == JoinMode.CreateIfNotExists ? JoinType.JoinOrCreateRoom : JoinType.JoinRoom;
                State = ClientState.Joining;
            }
        ...
        public bool OpSetCustomPropertiesOfActor(int actorNr, Hashtable propertiesToSet, Hashtable expectedProperties = null, WebFlags webFlags = null) {
            ...
            if (CurrentRoom == null) {
                ...
                if (expectedProperties == null && webFlags == null && LocalPlayer != null && LocalPlayer.ActorNumber == actorNr) return LocalPlayer.SetCustomProperties(propertiesToSet);

                if (LoadBalancingPeer.DebugOut >= DebugLevel.ERROR) DebugReturn(DebugLevel.ERROR, "OpSetCustomPropertiesOfActor() failed. To use expectedProperties or webForward, you have to be in a room. State: " + State);
                return false;
            }

            Hashtable customActorProperties = new();
        ...
        protected internal bool OpSetPropertiesOfActor(int actorNr, Hashtable actorProperties, Hashtable expectedProperties = null, WebFlags webFlags = null) {
            ...
            bool res = LoadBalancingPeer.OpSetPropertiesOfActor(actorNr, actorProperties, expectedProperties, webFlags);
            if (res && !CurrentRoom.BroadcastPropertiesChangeToAll && (expectedProperties == null || expectedProperties.Count == 0)) {
        ...
        protected internal bool OpSetPropertiesOfRoom(Hashtable gameProperties, Hashtable expectedProperties = null, WebFlags webFlags = null) {
            ...
            bool res = LoadBalancingPeer.OpSetPropertiesOfRoom(gameProperties, expectedProperties, webFlags);
            if (res && !CurrentRoom.BroadcastPropertiesChangeToAll && (expectedProperties == null || expectedProperties.Count == 0)) {
        ...
        public void ChangeLocalID(int newID) {
            if (LocalPlayer == null) DebugReturn(DebugLevel.WARNING, string.Format("Local actor is null or not in mActors! mLocalActor: {0} mActors==null: {1} newID: {2}", LocalPlayer, CurrentRoom.Players == null, newID));
        ...
        private void GameEnteredOnGameServer(OperationResponse operationResponse) {
            ...
            object temp;
            if (operationResponse.Parameters.TryGetValue(ParameterCode.RoomOptionFlags, out temp)) CurrentRoom.InternalCacheRoomFlags((int)temp);
            ...
            if (CurrentRoom.SuppressRoomEvents) {
                if (lastJoinType == JoinType.CreateRoom || (lastJoinType == JoinType.JoinOrCreateRoom && LocalPlayer.ActorNumber == 1)) MatchMakingCallbackTargets.OnCreatedRoom();
        ...
        private void UpdatedActorList(int[] actorsInGame) {
            if (actorsInGame != null)
                foreach (int actorNumber in actorsInGame) {
                    ...
                    Player target = CurrentRoom.GetPlayer(actorNumber);
        ...
        protected internal virtual Player CreatePlayer(string actorName, int actorNumber, bool isLocal, Hashtable actorProperties) {
            Player newPlayer = new(actorName, actorNumber, isLocal, actorProperties);
            return newPlayer;
        }

        /// <summary>Internal "factory" method to create a room-instance.</summary>
        protected internal virtual Room CreateRoom(string roomName, RoomOptions opt) {
            Room r = new(roomName, opt);
            return r;
        }
        ...
        private bool CheckIfOpCanBeSent(byte opCode, ServerConnection serverConnection, string opName) {
            ...
            if (!CheckIfOpAllowedOnServer(opCode, serverConnection)) {
                if (LoadBalancingPeer.DebugOut >= DebugLevel.ERROR) DebugReturn(DebugLevel.ERROR, string.Format("Operation {0} ({1}) not allowed on current server ({2})", opName, opCode, serverConnection));
                return false;
            }

            if (!CheckIfClientIsReadyToCallOperation(opCode)) {
                DebugLevel levelToReport = DebugLevel.ERROR;
                if (opCode == OperationCode.RaiseEvent && (State == ClientState.Leaving || State == ClientState.Disconnecting || State == ClientState.DisconnectingFromGameServer)) levelToReport = DebugLevel.INFO;

                if (LoadBalancingPeer.DebugOut >= levelToReport) DebugReturn(levelToReport, string.Format("Operation {0} ({1}) not called because client is not connected or not ready yet, client state: {2}", opName, opCode, Enum.GetName(typeof(ClientState), State)));

                return false;
            }

            if (LoadBalancingPeer.PeerState != PeerStateValue.Connected) {
                DebugReturn(DebugLevel.ERROR, string.Format("Operation {0} ({1}) can't be sent because peer is not connected, peer state: {2}", opName, opCode, LoadBalancingPeer.PeerState));
                return false;
            }
        ...
        private void CallbackRoomEnterFailed(OperationResponse operationResponse) {
            if (operationResponse.ReturnCode != 0) {
                ...
                else if (operationResponse.OperationCode == OperationCode.JoinRandomGame) MatchMakingCallbackTargets.OnJoinRandomFailed(operationResponse.ReturnCode, operationResponse.DebugMessage);
            }
        }
        ...
        public virtual void OnOperationResponse(OperationResponse operationResponse) {
            ...
            if (operationResponse.Parameters.ContainsKey(ParameterCode.Token)) {
                ...
                AuthValues.Token = operationResponse.Parameters[ParameterCode.Token];
                tokenCache = AuthValues.Token;
            }

            // if the operation limit was reached, disconnect (but still execute the operation response).
            if (operationResponse.ReturnCode == ErrorCode.OperationLimitReached) Disconnect(DisconnectCause.DisconnectByOperationLimit);
            ...
            switch (operationResponse.OperationCode) {
                ...
                case OperationCode.AuthenticateOnce: {
                    ...
                    if (Server == ServerConnection.NameServer || Server == ServerConnection.MasterServer) {
                        ...
                        if (operationResponse.Parameters.ContainsKey(ParameterCode.EncryptionData)) SetupEncryption((Dictionary<byte, object>)operationResponse.Parameters[ParameterCode.EncryptionData]);
                    }
                    ...
                    if (Server == ServerConnection.NameServer) {
                        ...
                        if (AuthMode == AuthModeOption.AuthOnceWss && ExpectedProtocol != null) {
                            DebugReturn(DebugLevel.INFO, string.Format("AuthOnceWss mode. Auth response switches TransportProtocol to ExpectedProtocol: {0}.", ExpectedProtocol));
                        ...
                        if (enterRoomParamsCache.JoinMode == JoinMode.RejoinOnly) {
                            ...
                            Hashtable allProps = new();
                            allProps.Merge(LocalPlayer.CustomProperties);

                            if (!string.IsNullOrEmpty(LocalPlayer.NickName)) allProps[ActorProperties.PlayerName] = LocalPlayer.NickName;
                        ...
                        else if (lastJoinType == JoinType.CreateRoom) LoadBalancingPeer.OpCreateRoom(enterRoomParamsCache);
                        break;
                    }
                ...
                case OperationCode.GetGameList:
                    ...
                    List<RoomInfo> _RoomInfoList = new();

                    Hashtable games = (Hashtable)operationResponse[ParameterCode.GameList];
                    foreach (string gameName in games.Keys) _RoomInfoList.Add(new RoomInfo(gameName, (Hashtable)games[gameName]));
                ...
                case OperationCode.FindFriends:
                    ...
                    List<FriendInfo> friendList = new(friendListRequested.Length);
            ...
            switch (statusCode) {
                case StatusCode.Connect:
                    if (State == ClientState.ConnectingToNameServer) {
                        if (LoadBalancingPeer.DebugOut >= DebugLevel.ALL) DebugReturn(DebugLevel.ALL, "Connected to nameserver.");

                        Server = ServerConnection.NameServer;
                        if (AuthValues != null) AuthValues.Token = null; // when connecting to NameServer, invalidate the secret (only)
                    }

                    if (State == ClientState.ConnectingToGameServer) {
                        if (LoadBalancingPeer.DebugOut >= DebugLevel.ALL) DebugReturn(DebugLevel.ALL, "Connected to gameserver.");

                        Server = ServerConnection.GameServer;
                    }

                    if (State == ClientState.ConnectingToMasterServer) {
                        if (LoadBalancingPeer.DebugOut >= DebugLevel.ALL) DebugReturn(DebugLevel.ALL, "Connected to masterserver.");
                    ...
                    if (LoadBalancingPeer.TransportProtocol != ConnectionProtocol.WebSocketSecure) {
                        if (Server == ServerConnection.NameServer || AuthMode == AuthModeOption.Auth) LoadBalancingPeer.EstablishEncryption();
                    }
                    else {
                ...
                case StatusCode.Disconnect:
                    ...
                    if (ExpectedProtocol != null && LoadBalancingPeer.TransportProtocol != ExpectedProtocol) {
                        DebugReturn(DebugLevel.INFO, string.Format("On disconnect switches TransportProtocol to ExpectedProtocol: {0}.", ExpectedProtocol));
                        LoadBalancingPeer.TransportProtocol = (ConnectionProtocol)ExpectedProtocol;
                        ExpectedProtocol = null;
                    }
                    ...
                    switch (State) {
                        case ClientState.ConnectWithFallbackProtocol:
                            ...
                            LoadBalancingPeer.TransportProtocol = LoadBalancingPeer.TransportProtocol == ConnectionProtocol.Tcp ? ConnectionProtocol.Udp : ConnectionProtocol.Tcp;
                            NameServerPortInAppSettings = 0; // this does not affect the ServerSettings file, just a variable at runtime
                            ServerPortOverrides = new PhotonPortDefinition(); // use default ports for the fallback

                            if (!LoadBalancingPeer.Connect(NameServerAddress, ProxyServerAddress, AppId, TokenForInit)) return;
                            State = ClientState.ConnectingToNameServer;
                            break;
                        case ClientState.PeerCreated:
                        case ClientState.Disconnecting:
                            if (AuthValues != null) AuthValues.Token = null; // when leaving the server, invalidate the secret (but not the auth values)
                        ...
                        case ClientState.DisconnectingFromMasterServer:
                            Connect(GameServerAddress, ProxyServerAddress, ServerConnection.GameServer); // this connects the client with the Game Server (when joining/creating a room)
                            break;
            ...
            switch (photonEvent.Code) {
                ...
                case EventCode.Join:
                    ...
                    if (originatingPlayer == null) {
                        ...
                        originatingPlayer.HasRejoined = actorNr != LocalPlayer.ActorNumber; // event is for non-local player, who is known (by ActorNumber), so it's a returning player
                    }
                ...
                case EventCode.Leave:
                    if (originatingPlayer != null) {
                        ...
                        if (photonEvent.Parameters.ContainsKey(ParameterCode.IsInactive)) isInactive = (bool)photonEvent.Parameters[ParameterCode.IsInactive];

                        originatingPlayer.IsInactive = isInactive;
                        originatingPlayer.HasRejoined = false;

                        if (!isInactive) CurrentRoom.RemovePlayer(actorNr);
                    }
                ...
                case EventCode.PropertiesChanged:
                    ...
                    int targetActorNr = 0;
                    if (photonEvent.Parameters.ContainsKey(ParameterCode.TargetActorNr)) targetActorNr = (int)photonEvent[ParameterCode.TargetActorNr];
                ...
                case EventCode.AuthEvent:
                    ...
                    AuthValues.Token = photonEvent[ParameterCode.Token];
                    tokenCache = AuthValues.Token;
                    break;
            }
        ...
        private void OnDisconnectMessageReceived(DisconnectMessage obj) {
            DebugReturn(DebugLevel.ERROR, string.Format("Got DisconnectMessage. Code: {0} Msg: \"{1}\". Debug Info: {2}", obj.Code, obj.DebugMessage, obj.Parameters.ToStringFull()));
        ...
        protected internal static string ReplacePortWithAlternative(string address, ushort replacementPort) {
            ...
            if (webSocket) {
                ...
                UriBuilder urib = new(string.Format("scheme://{0}", address));
        ...
        protected internal void UpdateCallbackTargets() {
            while (callbackTargetChanges.Count > 0) {
                ...
                UpdateCallbackTarget<IErrorInfoCallback>(change, ErrorInfoCallbackTargets);
    ...
    public class ConnectionCallbacksContainer : List<IConnectionCallbacks>, IConnectionCallbacks {
        ...
        public void OnConnected() {
            ...
            foreach (IConnectionCallbacks target in this) target.OnConnected();
        }

        public void OnConnectedToMaster() {
            client.UpdateCallbackTargets();

            foreach (IConnectionCallbacks target in this) target.OnConnectedToMaster();
        }

        public void OnRegionListReceived(RegionHandler regionHandler) {
            client.UpdateCallbackTargets();

            foreach (IConnectionCallbacks target in this) target.OnRegionListReceived(regionHandler);
        }

        public void OnDisconnected(DisconnectCause cause) {
            client.UpdateCallbackTargets();

            foreach (IConnectionCallbacks target in this) target.OnDisconnected(cause);
        }

        public void OnCustomAuthenticationResponse(Dictionary<string, object> data) {
            client.UpdateCallbackTargets();

            foreach (IConnectionCallbacks target in this) target.OnCustomAuthenticationResponse(data);
        }

        public void OnCustomAuthenticationFailed(string debugMessage) {
            client.UpdateCallbackTargets();

            foreach (IConnectionCallbacks target in this) target.OnCustomAuthenticationFailed(debugMessage);
        }
    }
    ...
    public class MatchMakingCallbacksContainer : List<IMatchmakingCallbacks>, IMatchmakingCallbacks {
        ...
        public void OnCreatedRoom() {
            ...
            foreach (IMatchmakingCallbacks target in this) target.OnCreatedRoom();
        }

        public void OnJoinedRoom() {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnJoinedRoom();
        }

        public void OnCreateRoomFailed(short returnCode, string message) {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnCreateRoomFailed(returnCode, message);
        }

        public void OnJoinRandomFailed(short returnCode, string message) {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnJoinRandomFailed(returnCode, message);
        }

        public void OnJoinRoomFailed(short returnCode, string message) {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnJoinRoomFailed(returnCode, message);
        }

        public void OnLeftRoom() {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnLeftRoom();
        }

        public void OnFriendListUpdate(List<FriendInfo> friendList) {
            client.UpdateCallbackTargets();

            foreach (IMatchmakingCallbacks target in this) target.OnFriendListUpdate(friendList);
        }
    }
    ...
    internal class InRoomCallbacksContainer : List<IInRoomCallbacks>, IInRoomCallbacks {
        ...
        public void OnPlayerEnteredRoom(Player newPlayer) {
            ...
            foreach (IInRoomCallbacks target in this) target.OnPlayerEnteredRoom(newPlayer);
        }

        public void OnPlayerLeftRoom(Player otherPlayer) {
            client.UpdateCallbackTargets();

            foreach (IInRoomCallbacks target in this) target.OnPlayerLeftRoom(otherPlayer);
        }

        public void OnRoomPropertiesUpdate(Hashtable propertiesThatChanged) {
            client.UpdateCallbackTargets();

            foreach (IInRoomCallbacks target in this) target.OnRoomPropertiesUpdate(propertiesThatChanged);
        }

        public void OnPlayerPropertiesUpdate(Player targetPlayer, Hashtable changedProp) {
            client.UpdateCallbackTargets();

            foreach (IInRoomCallbacks target in this) target.OnPlayerPropertiesUpdate(targetPlayer, changedProp);
        }

        public void OnMasterClientSwitched(Player newMasterClient) {
            client.UpdateCallbackTargets();

            foreach (IInRoomCallbacks target in this) target.OnMasterClientSwitched(newMasterClient);
        }
    }
    ...
    internal class LobbyCallbacksContainer : List<ILobbyCallbacks>, ILobbyCallbacks {
        ...
        public void OnJoinedLobby() {
            ...
            foreach (ILobbyCallbacks target in this) target.OnJoinedLobby();
        }

        public void OnLeftLobby() {
            client.UpdateCallbackTargets();

            foreach (ILobbyCallbacks target in this) target.OnLeftLobby();
        }

        public void OnRoomListUpdate(List<RoomInfo> roomList) {
            client.UpdateCallbackTargets();

            foreach (ILobbyCallbacks target in this) target.OnRoomListUpdate(roomList);
        }

        public void OnLobbyStatisticsUpdate(List<TypedLobbyInfo> lobbyStatistics) {
            client.UpdateCallbackTargets();

            foreach (ILobbyCallbacks target in this) target.OnLobbyStatisticsUpdate(lobbyStatistics);
        }
    }
    ...
    internal class WebRpcCallbacksContainer : List<IWebRpcCallback>, IWebRpcCallback {
        ...
        public void OnWebRpcResponse(OperationResponse response) {
            ...
            foreach (IWebRpcCallback target in this) target.OnWebRpcResponse(response);
        }
    }
    ...
    internal class ErrorInfoCallbacksContainer : List<IErrorInfoCallback>, IErrorInfoCallback {
        ...
        public void OnErrorInfo(ErrorInfo errorInfo) {
            client.UpdateCallbackTargets();
            foreach (IErrorInfoCallback target in this) target.OnErrorInfo(errorInfo);
        }
    }
